...
export const normalizeCategory = (cat, schemeName = '') => {
    ...
    // Hybrid / Arbitrage - Critical for Equity Alignment
    if (/hybrid|balanced|arbitrage/i.test(lowerFull)) {
        if (/arbitrage/i.test(lowerFull)) return 'Hybrid - Arbitrage';
        if (/balanced advantage|dynamic asset/i.test(lowerFull)) return 'Hybrid - Balanced Advantage';
        return 'Hybrid';
    }
