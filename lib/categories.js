...
export function getFundsForCategory(apiCategory) {
    ...
    if (lower.includes('hybrid')) return SAMPLE_FUNDS['Equity - Large & Mid Cap'] || []; // Fallback for Hybrid
    if (lower.includes('arbitrage')) return SAMPLE_FUNDS['Equity - Large Cap'] || []; // Lowest risk equity fallback
    return [];
}
