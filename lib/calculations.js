...
export function calculateCAGR(startValue, endValue, years) {
    if (!startValue || !endValue || !years || isNaN(startValue) || isNaN(endValue) || isNaN(years)) {
        return 0; // Return 0 instead of NaN to keep UI stable
    }
    if (startValue <= 0 || years <= 0 || endValue <= 0) return 0;
    try {
        const growth = endValue / startValue;
        const res = (Math.pow(growth, 1 / years) - 1) * 100;
...
export function calculateAbsoluteReturn(startValue, endValue) {
    if (!startValue || isNaN(startValue) || isNaN(endValue)) return 0;
    if (startValue <= 0) return 0;
    const res = ((endValue - startValue) / startValue) * 100;
    return isFinite(res) ? res : 0;
}
