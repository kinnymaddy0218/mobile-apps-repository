...
class ScraperEngine {
    ...
    async getDiscoveryQueue(batchSize = 20) {
        if (!db) return [];
        try {
            console.log(`[Scraper] Generating Discovery Queue (Batch Size: ${batchSize})...`);
            
            // Priority 1: Funds flagged with 'needsDiscovery' (User failures)
            const p1Query = query(
            ...
            if (remaining > 0) {
                console.log(`[Scraper] Universe Discovery: Picking up to ${remaining} funds from Global Index...`);
                const index = await this.fetchIndex();
                
                // Shuffle for random sampling across the market
                const sample = index.sort(() => 0.5 - Math.random()).slice(0, remaining * 3);
                
                for (const fund of sample) {
            ...
            // Priority 3: Stale Maintenance (> 30 days)
            if (queue.length < batchSize) {
                const finalRemaining = batchSize - queue.length;
                const staleCutoff = new Date();
                staleCutoff.setDate(staleCutoff.getDate() - 30);
                
                const p3Query = query(
                    collection(db, 'fund_factsheets'),
                    where('lastChecked', '<', staleCutoff.toISOString()),
                    orderBy('lastChecked', 'asc'),
                    limit(finalRemaining)
                );
                
                const p3Snap = await getDocs(p3Query);
                p3Snap.docs.forEach(d => {
                    if (!queue.find(item => item.instId === d.id)) {
                        queue.push({ 
                            instId: d.id, 
                            fundName: d.data().fundName,
